...
async def send_movie_to_user(context: ContextTypes.DEFAULT_TYPE, user_id: int, movie_data: tuple, mode="auto"):
    ...
    mode="season" -> Shows Qualities for a Season
    ...
    try:
        ...
        if mode == "auto":
            ...
            if is_series:
                ...
                if seasons:
                    # Check if any file has no episode (complete season)
                    has_complete_season = any(parse_movie_info(m[1])['episode'] is None for m in all_files)

                    if has_complete_season:
                        # Show quality options for complete season
                        keyboard = []
                        quality_map = {}

                        for m in all_files:
                            m_info = parse_movie_info(m[1])
                            if m_info['episode'] is None:  # Complete season
                                key = f"{m_info['quality']}_{m_info['language']}"
                                if key not in quality_map:
                                    quality_map[key] = m

                        for key, m in quality_map.items():
                            quality, language = key.split('_')
                            btn_text = f"📁 {quality} {language}"
                            keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"select_quality_{m[0]}")])

                        if keyboard:
                            await context.bot.send_message(
                                chat_id=chat_id,
                                text=f"🎬 <b>{base_name.title()}</b>\n\n📌 Complete Season Available\n👇 Select Quality:",
                                reply_markup=InlineKeyboardMarkup(keyboard),
                                parse_mode='HTML'
                            )
                            return

                    # If no complete season, show season selection
                    keyboard = []
                    row = []
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # --- SELECT QUALITY (for movies or complete seasons) ---
        ...
        # --- SELECT SEASON (for series) ---
            ...
            if conn:
                ...
                if res:
                    ...
                    for m in all_files:
                        m_info = parse_movie_info(m[1])
                        if m_info['season'] == season_num:
                            season_files.append(m)

                    if season_files:
                        # Group by quality and language
                        quality_map = {}
                        for m in season_files:
                            m_info = parse_movie_info(m[1])
                            key = f"{m_info['quality']}_{m_info['language']}"
                            if key not in quality_map:
                                quality_map[key] = m
                        ...
                        await query.message.edit_text("❌ No files found for this season.")
                else:
...
4. वहां आपको सीरीज के लिए सीज़न और क्वालिटी चुनने का विकल्प मिलेगा
...
async def private_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if movie_data:
        ...
        await update.message.reply_text("❌ Movie not found. Please try with a different name.")
